...
def daily_trader_agg(df: pd.DataFrame) -> pd.DataFrame:
    ...
    cols = set(df.columns)
    pnl_col = next((c for c in ["closedpnl", "pnl", "realizedpnl"] if c in cols), None)
    size_col = next((c for c in ["size", "qty", "quantity"] if c in cols), None)
    ...
    out["trades"] = g.size()
    if pnl_col:
        out["pnl"] = g[pnl_col].sum(min_count=1)
    if size_col:
        out["volume"] = g[size_col].abs().sum(min_count=1)
    if side_col:
